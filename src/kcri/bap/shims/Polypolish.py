#!/usr/bin/env python3
#
# kcri.bap.shims.Polypolish - service shim to the Polypolish backend
#
...
SERVICE, VERSION = "Polypolish", BACKEND_VERSIONS['polypolish']

# Polypolish resource parameters: cpu, memory, disk, run time reqs
#MAX_CPU = -1 # all
#MAX_MEM = 12 # all
MAX_TIM = 0  # unlimited
...
# The Service class
class PolypolishShim:
    '''Service shim that executes the Polypolish backend.'''

    def execute(self, sid, xid, blackboard, scheduler):
        '''Invoked by the executor.  Creates, starts and returns the Task.'''

        execution = PolypolishExecution(SERVICE, VERSION, sid, xid, blackboard, scheduler)
...
class PolypolishExecution(ServiceExecution):
    ...
    def start(self, job_spec):
        if self.state == Task.State.STARTED:
            self._job = self._scheduler.schedule_job('polypolish', job_spec, 'Polypolish')
